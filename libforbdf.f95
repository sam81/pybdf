...
subroutine read_channels(filename, starttime, endtime, nChannels, nSampRec, statusChanIdx, dats, statchan)
  ...
  integer*4 :: bitstr, bitstr1, bitstr2, bitstr3
  integer*2 :: int16
  ...
  integer*2, intent(out) :: statchan(0:1, 0:(nSampRec*(endtime-starttime))-1)
  ...
  do k=0,(nRec-1)
     do ch=0,(nChannels-1)
        if (ch .NE. statusChanIdx) then
           ...
           do m=0,(nSampRec-1)
           bitstr1 = TRANSFER(SOURCE=datchar(cpos)//datchar(cpos+1), MOLD=int16)
           bitstr2 = TRANSFER(SOURCE=datchar(cpos+2)//onechar, MOLD=int16)
